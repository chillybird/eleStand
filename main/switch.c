...
bool sw_is_down(void) {
    return gpio_get_level(SW_DOWN_GPIO) == 0; 
}

bool sw_is_up(void) { 
    return gpio_get_level(SW_UP_GPIO) == 0; 
}
...
bool sw_calibrate(void) {
    ...
    if (hit2 == hit1) {
        ESP_LOGW("sw", "Both directions hit GPIO%d", hit1);
    } else {
        ESP_LOGI("sw", "CCW -> GPIO%d", hit2);
    }
